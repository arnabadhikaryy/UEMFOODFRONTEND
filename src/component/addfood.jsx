...
const AddFoodPage = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    availability: true,
    discount: 0,
    shop_name: 'Hungry Baba',
    shop_address: 'CHANDPARA',
  ...
  const categories = ['Food', 'Momo & Rool', 'Noodles', 'Veg Starters', 'Non-veg Starters', 'Sandwich & Pasta', 'Rice & Combo', 'Coffee'];
  const shopTypes = ['Restaurant', 'Cafe', 'Fast Food Joint', 'Food Truck', 'Bakery', 'Bar', 'Pub'];
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setFormData(prev => ({ ...prev, pic_url: '' }));
    }
  };

  const handleUrlChange = (e) => {
    const url = e.target.value;
    setFormData(prev => ({ ...prev, pic_url: url }));
    if (url) {
      setPreview(url);
      setImageFile(null);
    } else {
  ...
  const handleSubmit = async (e) => {
    ...
    if (uploadMethod === 'url' && !formData.pic_url) {
    ...
    try {
      ...
      Object.keys(formData).forEach(key => {
        if (key !== 'pic_url' || uploadMethod === 'url') {
          formDataToSend.append(key, formData[key]);
        }
      });
      
      // Append file if upload method is file
      if (uploadMethod === 'file' && imageFile) {
      ...
      <div className="flex-grow flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100">
            {/* Header */}
            <div className="bg-gradient-to-r from-emerald-600 to-emerald-700 px-8 py-6 relative overflow-hidden">
              <div className="relative z-10">
                <h1 className="text-3xl font-bold text-white tracking-tight">Add New Food Item</h1>
                <p className="text-emerald-100 mt-1 text-sm">Complete all details to list your food item</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {uploadMethod === 'file' ? (
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 transition-all outline-none"
                      />
                      {preview && (
                        <div className="mt-4 relative h-56 rounded-xl overflow-hidden border border-gray-200">
                          <img src={preview} alt="Preview" className="w-full h-full object-cover" />
                        </div>
                      )}
                    </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                  <label className="flex items-center gap-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm font-semibold text-gray-700">Item is available</span>
                  </label>
                </motion.div>
                ...
                <div className="border-t border-gray-200 pt-6 mt-4">
                  ...
                  <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                      <label htmlFor="shop_name" className="block text-sm font-semibold text-gray-700 mb-2">
                        Shop Name
                      </label>
                      <input
                        type="text"
                    ...
                    <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.6 }}>
                      <label htmlFor="shop_type" className="block text-sm font-semibold text-gray-700 mb-2">
                        Shop Type
                      </label>
                      <select
                        id="shop_type"
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="mt-4">
                    <label htmlFor="shop_address" className="block text-sm font-semibold text-gray-700 mb-2">
                      Shop Address
                    </label>
                    <input
                      type="text"
                  ...
                  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="mt-4">
                    <label htmlFor="shop_image" className="block text-sm font-semibold text-gray-700 mb-2">
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.9 }}>
                  <label htmlFor="adminpassword" className="block text-sm font-semibold text-gray-700 mb-2">
                    Admin Authorization <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
