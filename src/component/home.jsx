...
const MenuPage = () => {
  ...
  // Function to fetch initial food items (first page)
  const fetchInitialFoodItems = async () => {
    try {
      ...
      const response = await axios.post(`${backend_Url}/production/getallfood_pagination`, {
      ...
      if (response.data.status) {
        ...
        setFoodItems(items);
        setLastProductId(response.data.next_last_product_id);
        setHasMore(items.length > 0 && response.data.next_last_product_id !== null);
  ...
  // Function to load more food items
  const loadMoreItems = useCallback(async () => {
    if (loadingMore || !hasMore || !lastProductId) return;
    ...
    try {
      ...
      if (response.data.status) {
        const newItems = response.data.message;
        if (newItems.length > 0) {
          setFoodItems(prevItems => [...prevItems, ...newItems]);
          setLastProductId(response.data.next_last_product_id);
          setHasMore(response.data.next_last_product_id !== null);

          // Update shops if new unique shops are found
          const uniqueShopsMap = new Map(shops.map(shop => [shop.name, shop.image]));
          ...
          const updatedShopsArray = Array.from(uniqueShopsMap, ([name, image]) => ({ name, image }));
          setShops(updatedShopsArray);
        } else {
          setHasMore(false);
        }
      }
  ...
  }, [lastProductId, loadingMore, hasMore, shops]);
  ...
  }, [autoLoadEnabled, hasMore, loadingMore, loadMoreItems]);

  // 1. Fetch Initial Food Items
  useEffect(() => {
  ...
  const toggleAutoLoad = () => {
    ...
    if (!autoLoadEnabled) {
      toast.success('Auto-load enabled! Scroll down to load more.');
    } else {
      toast.success('Auto-load disabled! Use "Load More" button.');
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {!loading && shops.length > 0 && !isNetworkError && (
          <div className="mb-12">
            ...
            <div className="flex bg-amber-200 justify-center h-20 items-center gap-4 mb-4">
              <button
                onClick={toggleAutoLoad}
                className={` px-6 py-2.5 rounded-xl font-semibold transition-all duration-300 flex items-center gap-2 ${autoLoadEnabled
                    ? 'bg-emerald-500 text-blue-600 hover:bg-emerald-600 shadow-md'
        ...
        {(loading || isFiltering) && !error && !isNetworkError ? (
          ...
          <>
            ...
            <div className="mt-12 flex flex-col items-center gap-4">
              ...
              {/* Load More Button (visible when auto-load is off) */}
              {!autoLoadEnabled && hasMore && (
                <button
                  onClick={loadMoreItems}
                  disabled={loadingMore}
                  className="px-8 py-3 bg-emerald-500 hover:bg-emerald-600 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold rounded-xl transition-all duration-300 shadow-md hover:shadow-lg flex items-center gap-2"
                >
                  {loadingMore ? (
                    <>
                    ...
                    <>
                      <div className=' text-black' >Load More Products</div>
                    </>
                  )}
                </button>
              )}
              ...
              {autoLoadEnabled && loadingMore && (
                <div className="flex items-center gap-2 text-emerald-600 dark:text-emerald-400">
                  ...
                  <span className="font-semibold text-blue-700">Loading more products...</span>
                </div>
              )}
              ...
              {autoLoadEnabled && hasMore && <div ref={loadMoreRef} className="h-4" />}
            </div>
          </>
        ) : null}
      </div>
